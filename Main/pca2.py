...
class pca:
    ...
    def pca_eigen(self):
        ...
        ques = [
            {
                "type": "input",
                "name": "fname",
                "message": "enter name of .csv file to save eigen values",
            }
        ]
        ...
        for target, color in zip(labels, colors):
            ...
            graph.scatter(
                principalDf.loc[checkindex, "principal component 1"],
                principalDf.loc[checkindex, "principal component 2"],
                c=color,
                s=40,
            )
    ...
    def null_handler(self):
        ...
        )  # replacing empty spaces as numpy null NaN
        ...
        if col_name[0] != "all":
            if choice == "remove":
                ...
                for i in col_name:
                    if (
                        self.df.dtypes[i] == "clistnt64"
                        or self.df.dtypes[i] == "float64"
                    ):
